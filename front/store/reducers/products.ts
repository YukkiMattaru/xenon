...
type EditProductPayloadAction = Partial<Omit<XenonAPI.Product, 'id' | 'rating' | 'shopId' | 'oldPrice'>> &
    ...
    editProduct(state, action: PayloadAction<EditProductPayloadAction>) {
      state.products.forEach((product) => {
        if (product.id === action.payload.id) {
          const oldPrice =
            action.payload.price && product.price === action.payload.price ? product.oldPrice : product.price;
          product = {
            ...product,
            oldPrice,
            ...action.payload,
